...
namespace API.Controllers
{
    public class FutbollistiController : BaseApiController
    {
        ...
        [HttpGet]
        public ActionResult<List<Futbollisti>> GetFutbollistat()
        {
            var futbollistat = _context.Futbollisti.ToList();

            return Ok(futbollistat);
        }

        [HttpGet("{id}", Name = "GetFutbollisti")]
        ...
        {
            var emri = await _context.Futbollisti.Select(p => p.Emri).Distinct().ToListAsync();
            var skuadra = await _context.Futbollisti.Select(p => p.Skuadra).Distinct().ToListAsync();

            return Ok(new { emri, skuadra });
        }

        [HttpDelete("{id}")]
        public async Task<ActionResult> DeleteFutbollisti(int id)
        {
            var futbollisti = await _context.Futbollisti.FindAsync(id);
